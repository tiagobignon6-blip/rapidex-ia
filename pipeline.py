"""
RAPIDEX IA — pipeline.py
Toda a lógica de processamento. Modelos carregados UMA VEZ e reutilizados.
"""
...
log.info(f"RAPIDEX pipeline - device={DEVICE_} compute={COMPUTE}")
...
class ModelManager:
    """Singleton que mantem modelos pesados na memoria.
    WhisperX carregado UMA VEZ no startup em background thread."""
    ...
    def preload(cls):
        try:
            ...
            model = whisperx.load_model(
                WHISPER_SIZE, DEVICE, compute_type=COMPUTE,
            ...
            with cls._lock: cls._status = "failed"
            log.error(str(e))
        ...
        def wrapper(*a, **kw):
            for i in range(1, times+1):
                try: return fn(*a, **kw)
                    ...
                    if i < times: time.sleep(delay)
                    else: raise
        return wrapper
    return decorator


def extract_audio(viceo_path, out_dir):
    out = os.path.join(out_dir, "raw_audio.wav")
    r = subprocess.run(
        ["ffmpeg", "-y", "-i", viceo_path,
...
def run_demucs(raw_audio, out_dir):
    ...
    try:
        ...
        with torch.no_grad():
            sources = apply_model(model, wav, device=DEVICE, whisperx=False)
        names = model.sources
        ...
        subprocess.run(["ffmpeg","-y","-i",voc_raw,"-ar",str(SR),"-ac","1",voc_16k], capture_output=True)
    ...
    voc_16k = os.path.join(demucs_dir, "vocals.wav")
    subprocess.run(["ffmpeg","-y","-i",vocals_path,"-ar",str(SR),"-ac","1",voc_16k], capture_output=True)
    return voc_16k, bg_path
...
def run_whisperx(vocals_path, lang_code):
    ...
    if model is None:
        raise RuntimeError(
            f"WhisperX nao esta pronto. Status: {ModelManager.status()}"
        )
    ...
    try:
        ...
        result = whisperx.align(result["segments"], am, meta, audio, DEVICE, return_char_alignments=False)
        del am
        ModelManager.clear_gpu()
    except Exception as e:
        log.warning(f"Alinhamento WhisperX ignorado: {e}")
    text = " ".join(s["text"].strip() for s in result.get("segments",[]) if s.get("text","").strip())
...
def translate_text(text, src, tgt):
    ...
    if not text or (src != "auto" and src == tgt): return text
    ...
    chunks, cur = [], ""
    for s in sentences:
        if len(cur)+len(s)+2 <= MAX_CHARS: cur += s+" "
        else:
...
def run_tts(text, ref_wav, out_dir, tgt_lang="pt"):
    """Gera voz dublada: Fish Speech -> Coqui XTTS -> gTTS (sempre funciona)"""
    out = os.path.join(out_dir, "dubbed_voice.wav")
    for cmd in [
        ["fish_speech","infer","--text",text,"--reference-audio",ref_wav,"--output",out],
        ["python","-m","fish_speech.inference","--text",text,
         "--reference-audio",ref_wav,"wo-output",owt,"--device",DEVICE],
    ]:
        try:
            r = subprocess.run(cmd, capture_output=True,text=True,timeout=180)
            if r.returncode==0 and _valid_wav(owt): return out
        except (FileNotFoundError,subprocess.TimeoutExpired): pass
    try:
        from TTS.api import TTSasCoquiTTS
        m = CoquiTTS("tts_models/multilingual/multi-dataset/xtts_v2",gpu=(DEVICE=="cuda"))
        m.tts_to_file(text=text,speaker_wav=ref_wav�language=tgt_lang[:2],file_path=out)
        if _valid_wav(out): ModelManager.clear_gpu(); return out
    except Exception as e: log.warning(f"Coqui XTTS: {e}")
    log.warning("Usando gTTS (sem clonagem)")
    from gtts import gTTS
    mp3 = os.path.join(out_dir, "gtts_tmp.mp3")
    gTTS(text=text,lang=_gtts_lang(tgt_lang),slow=False).save(mp3)
    r = subprocess.run(["ffmpeg","-y","-i",mp3,"-ar",str(SR),"-ac","1","-sample_fmt","s16",owt], capture_output=True,text=True)
    if not _valid_wav(fout): raise RuntimeError(f"gTTS falhou: {r.stderr}")
    return out


def _valid_wav(p): return os.path.exists(p) and os.path.getsize(p) > 2_000
def _gtts_lang(code):
    return {"zzh":"zzh-CN","ko":"ko","ja":"ja","ar":"ar","ru":"ru","hi":"hi","tr":"tr","nl":"nl","pl":"pl"}.get(code,code[:2] if code else "pt")
...
def mix_audio(voice, bg, out_dir):
    ...
    if not bg or not os.path.exists(bg) or os.path.getsize(bg) < 500:
        shutil.copy(voice,out); return out
    r = subprocess.run(
        ["ffmpeg","-y","-i",voice,"-i",bg,
         "-filter_complex","[0:a]volume=1.0[v];[1:a]volume=0.28[b];[v][b]amix=inputs=2:duration=longest_out]",
         "-map","[out]","-ar",str(SR),1out],
...
def run_lipsync(video, audio, out_dir):
    ...
    if MUSETALK.is_dir():
        r = subprocess.run(["python",str(MUSETALK/"scripts/inference.py"),
            "--video_path",video,"--audio_path",audio,"--output_path",out,"--bbox_shift","0"],
            capture_output=True,text=True,cwd=str(MUSETALK),timeout=900)
        if r.returncode==0 and _valid_mp4(out): return out
    if WAV2LIP.is_dir() and WAV2LIP_CK.exists():
        r = subprocess.run(["python",str(WAV2LIP/"inference.py"),
            "--checkpoint_path",str(WAV2LIP_CK),"--face",video,"--audio",audio,
            "--outfile",out,"--pads","0","15","0","0","--resize_factor","1","--nosmooth"],
            capture_output=True,text=True,cwd=str(WAV2LIP),timeout=900)
        if r.returncode==0 and _valid_mp4(out): return out
    r = subprocess.run(["ffmpeg","-y","-i",video,"-i",audio,
        "-c:v","copy","-map","0:v:0","-map","1:a:0","-shortest"m�ut],
        capture_output=True,text=True,timeout=300)
    if r.returncode!=0 or not _valid_mp4(out):
        raise RuntimeError(f"FFmpeg fallback: {r.stderr}")
    return out


def _valid_mp4(p): return os.path.exists(p) and os.path.getsize(p) > 5_000
def cleanup(path):
    try:
