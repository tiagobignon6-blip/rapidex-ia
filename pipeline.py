...
def extract_audio(video_path, out_dir):
    ...
    log.info(f"Audio extraido: {out}")
    return out

# ── VOCAL SEPARATION (DEMUCS) ──────────────────────────────────────────────────

def run_demucs(raw_audio, out_dir):
    """Separa voz do fundo. Retorna (vocals_path, bg_path)."""
    demucs_dir = os.path.join(out_dir, "demucs")
    os.makedirs(demucs_dir, exist_ok=True)

    # Tentativa 1: API Python (mais precisa)
    try:
        ...
        # Normalizar (melhora qualidade de separacao)
        ref = wav.mean(0)
        wav = (wav - ref.mean()) / ref.std()

        wav_in = wav.unsqueeze(0)  # (1, channels, time)
    ...
    for root, _, files in os.walk(demucs_dir):
        if "vocals.wav" in files:
            ...
            break
    if not vocals_path:
        log.warning("Demucs falhou - usando audio bruto")
...
def run_tts(text, ref_wav, out_dir, tgt_lang="pt"):
    ...
    # 1. Fish Speech (clonagem de voz, melhor qualidade)
    for cmd in [
        ["fish_speech", "infer",
    ...
    try:
        ...
        lang_code = tgt_lang[:2] if tgt_lang else "pt"
        m.tts_to_file(
            text=text,
            speaker_wav=ref_wav,
            language=lang_code,
